...
diary.controller("feedbackController", function($scope, $http) {

    $scope.submitFeedback = function() {
        $scope.feedback_sent = "progress";
        $scope.feedback_submitted = true;
        if ($scope.feedbackContent) {
            $http.post('/diary/submit_feedback/', $scope.feedbackContent).then(
    ...
    this.saveMessage = function(message) {
        $http.post('/diary/add_message/', message).then(
            function(response) {
                ...
                tmp_messages = response.data.map(function(d) {
                    return new Message(angular.fromJson(d));
                });
